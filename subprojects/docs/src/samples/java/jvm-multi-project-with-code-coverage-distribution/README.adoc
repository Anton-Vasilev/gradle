...
The standalone project applies `jacoco-report-aggregation`, and requires no additional configuration if the `jvm-test-suite` plugin is present.
...
For unit tests, no additional configuration is necessary.  The aggregated reports and their backing tasks of type `JacocoReport` are synthesized based on the test suites existing in the `application` project.  Test suites having identical `testType` properties will be aggregated.
