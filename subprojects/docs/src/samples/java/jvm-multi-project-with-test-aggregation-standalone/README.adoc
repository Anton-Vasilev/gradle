...
====
include::sample[dir="groovy",files="test-results/build.gradle[]"]
include::sample[dir="kotlin",files="test-results/build.gradle.kts[]"]
====

The standalone project applies `test-report-aggregation`, but requires additional configuration if the `jvm-test-suite` plugin is not also present.
